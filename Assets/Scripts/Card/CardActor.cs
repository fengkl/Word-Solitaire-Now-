...
public class CardActor : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    [Header("卡面文字和移动相关")] 
    public float textScaleValue; // 文字缩放值
    public float spriteScaleValue; // 精灵缩放值
    ...
    public void MovingCardTextToBig()
    {
        ...
        if (useSprite)
        {
            sequence.Join(cardImage.transform.DOScale(originalScaleValue, moveDuration));
            sequence.Join(cardImage.transform.DOBlendableLocalMoveBy(new Vector3(0, -textMoveDistance, 0), moveDuration));
        }
        else
        {
