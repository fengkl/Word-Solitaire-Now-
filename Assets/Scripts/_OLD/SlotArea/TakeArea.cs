...
public class TakeArea : MonoBehaviour
{
    ...
    public GameObject takeAreaSlot; // 取牌区卡槽
    ...
    [Header("预制体")] 
    public GameObject takeCardSlotPrefab; //取牌区卡槽预制体
    public GameObject faceDownPrefab; //背面牌预制体
    ...
    public void Init()
    {
        takeAreaSlot = Instantiate(takeCardSlotPrefab, transform.position, Quaternion.identity);
        takeAreaSlot.transform.parent = transform;
        
        faceDownCount = Instantiate(faceDownPrefab, faceDownPrefab.transform.position, Quaternion.identity);
        faceDownCount.transform.SetParent(transform);
        countText = faceDownCount.GetComponentInChildren<TextMeshProUGUI>();
    }
    ...
    public void UpdateCardsPos()
    {
        ...
        count = cardGroups.Count < maxDisplayCount ? cardGroups.Count : maxDisplayCount;
        
        // 更新计数器位置
        Vector3 DisplayCountPosTarget = new Vector3(
            transform.position.x - 0.35f - (count - 1) * gapSize, 
            faceDownPrefab.transform.position.y, 
            count
            );
        
        faceDownCount.transform.DOMove(DisplayCountPosTarget, initMoveTime)
            .SetEase(Ease.InOutQuad);
